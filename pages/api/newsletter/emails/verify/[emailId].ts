...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const updatedNewsletter = await waitlistClient.updateOptIn(emailId as string, true)

  if (updatedNewsletter[0])
    return res.status(400).send({success: false, content: `error ${updatedNewsletter[0]}`})
